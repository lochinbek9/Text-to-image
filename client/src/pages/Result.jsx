    ...
    <form onSubmit={onSubmitHandler} className="flex flex-col min-h-[90vh] justify-center items-center">
      <div>
        <div className="relative">
          ...
          <span className={`absolute bottom-0 left-0 h-1 bg-blue-500 transition-all duration-[10s] ${loading ? 'w-full transition-all duration-[10s]' : "w-0"}`}></span>
        </div>
        <p className={!loading ? "hidden" : ""}>Loading...</p>
      </div>
      {!isImageLoaded && 
      <div className="flex w-full max-w-xl bg-neutral-500 text-white 
    text-sm py-0.5 mt-10 rounded-full">
        ...
        <button type="submit" className="bg-zinc-900 py-3 px-10 sm:px-3 rounded-full cursor-pointer">Generate</button>
      </div>
    }
